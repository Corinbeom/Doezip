...
export default defineConfig({
  ...
  webServer: [
    ...
    { command: 'node scripts/run.mjs start:api', env: { AUTH_ENABLED: 'true', AUTH_ISSUER: issuer, AUTH_JWK_SET_URI: `${issuer}/jwks`, AUTH_AUDIENCE: 'authenticated', AUTH_PROVIDER_ID: 'e2e-local', SUPABASE_AUTH_URL: `${issuer}/auth/v1`, SUPABASE_SECRET_KEY: 'e2e-secret-key', SPRING_PROFILES_ACTIVE: 'local,test' }, url: `http://localhost:${apiPort}/actuator/health`, reuseExistingServer: false, timeout: 120000, gracefulShutdown: { signal: 'SIGTERM', timeout: 10000 } },
