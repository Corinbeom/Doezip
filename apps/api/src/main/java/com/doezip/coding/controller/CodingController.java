...
public class CodingController {
 ...
 @PostMapping("/{id}/submit") public ResponseEntity<Workspace> submit(Authentication auth,@PathVariable UUID id,@Valid @RequestBody Submit body){service.get(current.id(auth),id);flowRecords.legacyOnly(id);return ok(service.submit(current.id(auth),id,body));}
