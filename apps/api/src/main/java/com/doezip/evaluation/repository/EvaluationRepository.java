...
public class EvaluationRepository {
 ...
 public record Job(UUID id,UUID sessionId,UUID documentId,String phase,String status,String snapshot,String fingerprint,UUID lease,int attempts,String error,Instant createdAt,UUID reportId){
  public boolean retryable(){return status.equals("FAILED")&&attempts<4&&Set.of("WORKER_LEASE_EXPIRED","WORKER_TEMPORARY_FAILURE","INVALID_EVALUATION_RESULT","AI_TIMEOUT","AI_AUTH_FAILED","AI_PROVIDER_UNAVAILABLE").contains(error==null?"":error);}
 ...
 public Job insert(UUID session,UUID task,UUID document,UUID challenge,UUID key,String snapshot,String hash){
  UUID id=UUID.randomUUID();db.update("INSERT INTO evaluation_runs(id,session_id,task_id,document_version_id,challenge_run_id,phase,idempotency_key,input_snapshot_json,input_fingerprint,evaluator_version,llm_config_json) VALUES (?,?,?,?,?,'INITIAL',?,?::jsonb,?,(?::jsonb)->>'evaluatorVersion',(?::jsonb)->'llmConfig')",id,session,task,document,challenge,key,snapshot,hash,snapshot,snapshot);return find(id).orElseThrow();
 }
 ...
 @Transactional public Optional<Job> claim(){
  UUID token=UUID.randomUUID();return db.query("WITH candidate AS (SELECT id FROM evaluation_runs WHERE status='QUEUED' AND next_attempt_at<=now() AND attempt_count<4 ORDER BY created_at,id FOR UPDATE SKIP LOCKED LIMIT 1) UPDATE evaluation_runs e SET status='RUNNING',attempt_count=attempt_count+1,lease_token=?,lease_expires_at=now()+interval '180 seconds',started_at=now(),error_code=NULL FROM candidate c WHERE e.id=c.id RETURNING e.*",this::map,token).stream().findFirst();
 }
 public boolean fail(Job job,String code,boolean temporary){return fail(job,code,temporary,0);}
 @Transactional public boolean fail(Job job,String code,boolean temporary,long retryAfterSeconds){
  long delay=Math.max(retryAfterSeconds,(job.attempts()<=1?5:20)+java.util.concurrent.ThreadLocalRandom.current().nextInt(3));
  return db.update("UPDATE evaluation_runs SET status=CASE WHEN ? AND attempt_count<3 THEN 'QUEUED' ELSE 'FAILED' END,error_code=?,lease_token=NULL,lease_expires_at=NULL,finished_at=clock_timestamp(),next_attempt_at=clock_timestamp()+(? * interval '1 second') WHERE id=? AND status='RUNNING' AND lease_token=? AND lease_expires_at>clock_timestamp()",temporary,code,delay,job.id(),job.lease())==1;
 }
 @Transactional public boolean heartbeat(Job job){return db.update("UPDATE evaluation_runs SET lease_expires_at=clock_timestamp()+interval '180 seconds' WHERE id=? AND status='RUNNING' AND lease_token=? AND lease_expires_at>clock_timestamp()",job.id(),job.lease())==1;}
