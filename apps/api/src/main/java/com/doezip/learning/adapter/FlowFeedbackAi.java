...
public class FlowFeedbackAi {
 ...
 public JsonNode evaluate(String model,JsonNode input){
  if(!input.path("promptVersion").asText().equals("learning-feedback-v1"))throw new IllegalArgumentException("unsupported prompt");
  try(var in=new ClassPathResource("learning/feedback-v1.txt").getInputStream();var client=Client.builder().apiKey(settings.apiKey()).vertexAI(false).httpOptions(HttpOptions.builder().timeout(60000).retryOptions(HttpRetryOptions.builder().attempts(1).build()).build()).build()){
