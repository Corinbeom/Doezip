...
it('explains the product before sending the user into the learning workspace',()=>{
  ...
  for(const link of screen.getAllByRole('link',{name:/^과제 둘러보기$/}))expect(link).toHaveAttribute('href','/tasks');
  ...
  expect(screen.getByRole('link',{name:/첫 과제 선택하기/})).toHaveAttribute('href','/tasks');
});
