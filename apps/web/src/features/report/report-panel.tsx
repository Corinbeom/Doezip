...
export function ReportPanel({id,userId}:{id:string;userId:string}){
 ...
 if(!query.data||query.isError)return <section aria-label="평가 결과"><p role={query.isError?'alert':'status'}>{query.isError?'평가 결과를 불러오지 못했습니다.':'평가 결과를 불러오는 중…'}</p>{query.isError&&<button onClick={()=>void query.refetch()}>결과 다시 불러오기</button>}</section>;
 const report=query.data;
 return <section aria-label="평가 결과"><h3>최초 평가 결과</h3>
