...
const feedbackAreaLabels={REQUEST:'문제와 요청 구체화',VERIFY:'제안 검증',IMPROVE:'결과물 개선',EXPLAIN:'판단 설명'} as const;
function HistoryCard({flow}:{flow:Flow}){
 ...
 const focus=flow.feedback&&feedbackAreaLabels[flow.feedback.practiceArea];
 const observation=flow.feedback?.items.find(item=>item.area===flow.feedback?.practiceArea)?.observation;
 return <article className={flow.stage==='FEEDBACK'?local.completedCard:undefined}><div><span>{flow.mode==='TRAINING'?'훈련':'모의 전형'}</span><span data-stage={flow.stage}>{stage}</span></div><h3>{flow.task.title}</h3><p>{flow.kind==='REPORT'?'보고서 작성 과제':'구현 과제'} · {flow.parentId?'재연습':'첫 수행'}</p>{flow.stage==='FEEDBACK'&&<div className={local.reportPreview}><span>학습 리포트</span><strong>{flow.feedbackStatus==='SUCCEEDED'&&focus?`${focus}을 다음 연습으로`:'피드백을 확인해 주세요'}</strong>{observation&&<p>{observation}</p>}</div>}<Link href={`/learn/${flow.id}`}>{action}<span aria-hidden="true"> →</span></Link></article>;
}
...
function Lobby({userId}:{userId:string}){
 ...
 return <>
  <section className={local.continueSection} aria-labelledby="continue-title"><div className={local.sectionTitle}><div><p>이어서 하기</p><h2 id="continue-title">진행 중인 학습</h2></div>{list.isSuccess&&<span>{activeFlows.length}개</span>}</div>
   {list.isPending?<p role="status" className={local.stateText}>학습 기록을 불러오는 중…</p>:list.isError?<div className={local.stateText}><p role="alert">학습 기록을 불러오지 못했습니다.</p><button onClick={()=>void list.refetch()}>목록 다시 불러오기</button></div>:activeFlows.length===0?<div className={local.emptyState}><p>진행 중인 과제가 없습니다.</p><span>새로운 연습은 과제 둘러보기에서 시작할 수 있습니다.</span><Link href="/tasks">과제 둘러보기 <span aria-hidden="true">→</span></Link></div>:<><div className={local.historyList}>{visibleActiveFlows.map(flow=><HistoryCard key={flow.id} flow={flow}/>)}</div>{remainingActiveFlows.length>0&&<details className={local.moreActive}><summary>진행 중 기록 {remainingActiveFlows.length}개 더 보기</summary><div className={local.historyList}>{remainingActiveFlows.map(flow=><HistoryCard key={flow.id} flow={flow}/>)}</div></details>}</>}
  </section>

  <section className={local.completedSection} aria-labelledby="completed-title"><div className={local.sectionTitle}><div><p>지난 기록</p><h2 id="completed-title">완료한 학습과 리포트</h2></div>{list.isSuccess&&<span>{completedFlows.length}개</span>}</div>{list.isPending?<p role="status" className={local.stateText}>완료한 기록을 불러오는 중…</p>:list.isSuccess&&(completedFlows.length===0?<div className={local.emptyState}><p>아직 완료한 과제가 없습니다.</p><span>과제를 마치면 결과물과 피드백 리포트가 이곳에 쌓입니다.</span></div>:<div className={local.historyList}>{completedFlows.map(flow=><HistoryCard key={flow.id} flow={flow}/>)}</div>)}</section>

  <aside className={local.catalogLink} aria-labelledby="catalog-link-title"><div><p>새로운 연습이 필요하신가요?</p><h2 id="catalog-link-title">과제 둘러보기에서 다음 과제를 선택하세요.</h2></div><Link href="/tasks">과제 둘러보기 <span aria-hidden="true">→</span></Link></aside>
 </>;
}
