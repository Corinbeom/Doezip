...
function Lobby({userId}:{userId:string}){
 ...
 return <>
  <section className={styles.panel}><h2>어떤 방식으로 연습할까요?</h2><label htmlFor="mode">과제 모드</label><select id="mode" value={mode} onChange={e=>setMode(e.target.value as typeof mode)}><option value="TRAINING">훈련 · 단계별 안내와 힌트</option><option value="SIMULATION">모의 전형 · 스스로 수행하고 제출 후 피드백</option></select><p>두 모드 모두 작업 AI를 사용할 수 있습니다. 제출 후에는 먼저 직접 설명하고, 피드백 이후 AI와 재연습합니다. 총점·등급·합격 예측은 제공하지 않습니다.</p></section>
  {error&&<p role="alert">{error}</p>}
  {tasks.isPending?<p>과제를 불러오는 중…</p>:tasks.isError?<button onClick={()=>void tasks.refetch()}>과제 다시 불러오기</button>:<div className={local.cards}>{tasks.data.map(t=><section className={styles.panel} key={t.kind}><h2>{t.title}</h2><p>{t.situation}</p><ul>{t.requirements.map(r=><li key={r}>{r}</li>)}</ul><p><strong>제출물:</strong> {t.deliverable}</p><p>요청 구체화 · 제안 검증 · 결과물 개선 · 판단 설명을 실제 기록으로 돌아봅니다.</p><button disabled={busy} onClick={()=>void start(t.kind)}>{t.kind==='REPORT'?'보고서 과제 시작':'구현 과제 시작'}</button></section>)}</div>}
  <section className={`${styles.panel} ${local.history}`}><div className={local.historyHeading}><div><p>저장된 수행 기록</p><h2>내 과제 이어하기</h2></div>{list.isSuccess&&<span>{list.data.length}개</span>}</div>
   {list.isPending?<p>불러오는 중…</p>:list.isError?<button onClick={()=>void list.refetch()}>목록 재시도</button>:list.data.length===0?<p>아직 시작한 과제가 없습니다.</p>:<div className={local.historyGroups}>
    <section className={local.historyGroup} aria-labelledby="active-flows-heading"><div className={local.historyGroupHeading}><h3 id="active-flows-heading">진행 중</h3><span>{activeFlows.length}개</span></div>{activeFlows.length===0?<p>진행 중인 과제가 없습니다.</p>:<div className={local.historyList}>{activeFlows.map(flow=><HistoryCard key={flow.id} flow={flow}/>)}</div>}</section>
    <details className={local.completedGroup}><summary><strong>완료한 과제</strong><span>{completedFlows.length}개 · 펼쳐 보기</span></summary><div className={local.historyList}>{completedFlows.map(flow=><HistoryCard key={flow.id} flow={flow}/>)}</div></details>
   </div>}
  </section>
 </>;
}
...
function Working({flow,userId,busy,act}:{flow:Flow;userId:string;busy:boolean;act:Act}){
 ...
 return <WorkStages busy={busy} training={flow.mode==='TRAINING'} kind={flow.kind} storageKey={flow.id} brief={<section className={styles.panel}><h3>{flow.task.title}</h3><p>{flow.task.situation}</p><ul>{flow.task.requirements.map(r=><li key={r}>{r}</li>)}</ul><p><strong>제출물:</strong> {flow.task.deliverable}</p><p>제출 전에는 단계 사이를 자유롭게 오갈 수 있습니다. 단계 이동은 제출이 아닙니다.</p></section>} work={<>{flow.mode==='TRAINING'&&<HintPanel kind={flow.kind} hints={flow.hints} busy={busy} onReveal={index=>void act(s=>changeFlow(`/${flow.id}/hints`,{index},'POST',s))}/>}<fieldset disabled={busy}>{flow.sessionId?(report.isPending?<p>작업 공간을 불러오는 중…</p>:report.isError?<button onClick={()=>void report.refetch()}>작업 공간 재시도</button>:<WorkspaceEditor workspace={report.data} userId={userId} flow onReady={readyState}/>):<LoadWorkspace id={flow.codingId!} userId={userId} flow onReady={readyState}/>}</fieldset></>} verification={<section className={styles.panel}><h2>내 결과물 검증과 제출</h2><label htmlFor="flow-explanation">채택한 방법과 남은 한계</label><textarea id="flow-explanation" maxLength={4000} value={notes.explanation} disabled={busy} onChange={e=>setNotes({...notes,explanation:e.target.value})}/><label htmlFor="flow-verification">무엇을 어떻게 확인했나요?</label><textarea id="flow-verification" maxLength={4000} value={notes.verification} disabled={busy} onChange={e=>setNotes({...notes,verification:e.target.value})} placeholder="주장과 원자료 또는 요구사항과 테스트 결과를 비교하세요. 확인하지 못했다면 그 사실과 다음 확인 방법을 적으세요."/>
