...
it('applies a reviewed proposal and allows undo without automatically saving',async()=>{mount();const editor=await screen.findByRole('textbox',{name:'solution.js'});fireEvent.click(screen.getByRole('button',{name:'이 수정안 적용'}));expect(editor).toHaveValue(workspace.turns[0].proposedCode);expect(mutate).not.toHaveBeenCalled();fireEvent.click(screen.getByRole('button',{name:'AI 변경 되돌리기'}));expect(editor).toHaveValue(original);});
it('does not apply an old proposal over unsaved edits',async()=>{mount();const editor=await screen.findByRole('textbox',{name:'solution.js'});fireEvent.change(editor,{target:{value:'my unsaved code'}});expect(screen.getByRole('button',{name:'이 수정안 적용'})).toBeDisabled();expect(editor).toHaveValue('my unsaved code');});
