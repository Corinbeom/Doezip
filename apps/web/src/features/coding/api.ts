...
export const workspaceSchema: z.ZodType<Workspace>=z.object({id:z.uuid(),taskVersion:z.literal('duplicate-items-v1'),code:z.string(),version:z.number().int().nonnegative(),submittedAt:z.string().nullable(),explanation:z.string().nullable(),lastRun:z.object({version:z.number(),suite:z.literal('duplicate-items-v1'),results:z.array(result)}).nullable(),turns:z.array(z.object({id:z.uuid(),requestKey:z.uuid(),baseVersion:z.number(),baseCode:z.string(),instruction:z.string(),status:z.enum(['RUNNING','SUCCEEDED','FAILED']),explanation:z.string().nullable(),proposedCode:z.string().nullable()}))});
