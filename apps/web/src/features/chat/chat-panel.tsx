...
export function ChatPanel({sessionId,userId,allowed,draftReady,onBusy}:{sessionId:string;userId:string;allowed:boolean;draftReady:boolean;onBusy:(busy:boolean)=>void}){
 const [text,setText]=useState('');const [include,setInclude]=useState(false);const [busy,setBusy]=useState(false);const [optimistic,setOptimistic]=useState<{text:string;afterSeq:number}|null>(null);const [live,setLive]=useState('');const [activeId,setActiveId]=useState<string|null>(null);const [error,setError]=useState('');const [retry,setRetry]=useState<'same'|'new'|null>(null);const [stopping,setStopping]=useState(false);const [suggestionsPreference,setSuggestionsPreference]=useState<boolean|null>(null);
 ...
 const messages=query.data?.items??[];const running=messages.find(m=>m.status==='STREAMING');const waiting=busy||!!running;const suggestionsOpen=suggestionsPreference??false;const pendingVisible=!!optimistic&&messages.some(message=>message.seqNo>optimistic.afterSeq&&message.role==='USER'&&message.contentText===optimistic.text);
 ...
 async function send(fresh=false){
  ...
  const abort=new AbortController();controller.current=abort;setBusy(true);setOptimistic({text:body.contentText,afterSeq:messages.at(-1)?.seqNo??0});setLive('');setActiveId(null);setError('');setRetry(null);
  ...
  finally{if(mounted.current){await query.refetch();if(mounted.current){setBusy(false);setOptimistic(null);setLive('');setActiveId(null);}}}
 }
 ...
 return <AiConversationPanel ariaLabel="AI와 분석하기" title="보고서 분석 대화" description="AI의 제안을 참고하되, 원자료와 대조한 뒤 내 보고서에 직접 작성하세요." toolbar={<button type="button" onClick={()=>void query.refetch()} disabled={query.isFetching}>대화 새로 불러오기</button>} composer={composer}>
  ...
  {optimistic&&!pendingVisible&&<AiMessage role="user" label="나" status="전송됨"><MessageContent text={optimistic.text}/></AiMessage>}
  {busy&&<AiMessage role="assistant" label="AI 코치" status={live?'답변 작성 중…':'응답 준비 중…'}>{live?<MessageContent text={live}/>:<AiResponseProgress context="report"/>}</AiMessage>}
