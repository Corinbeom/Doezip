...
export function WorkspaceEditor({workspace,userId,flow=false,onReady}:{workspace:Workspace;userId:string;flow?:boolean;onReady?:(ready:boolean,version:number,value:string)=>void}){
 ...
 useEffect(()=>{onReady?.(!chatBusy&&!draft.dirty&&draft.status==='saved',draft.snapshot.lockVersion,draft.snapshot.contentHash);},[chatBusy,draft.dirty,draft.status,draft.snapshot.lockVersion,draft.snapshot.contentHash,onReady]);
 ...
 const reference=<aside className={styles.panel}><h2>과제 자료</h2>{workspace.session.allowedActions.includes('READ_MATERIALS')?<>{workspace.materials.length===0?<p>공개된 자료가 없습니다.</p>:<><div className={styles.tabs}>{workspace.materials.map(item=><button key={item.id} aria-pressed={selected===item.id} onClick={()=>setSelected(item.id)}>{item.title}</button>)}</div>{selected&&<MaterialViewer key={selected} sessionId={workspace.session.id} userId={userId} materialId={selected}/>}</>}</>:<p>현재 단계에서는 자료를 열 수 없습니다.</p>}</aside>;
 const artifact=<section className={`${styles.panel} ${flow?styles.flowReportPanel:''}`} aria-labelledby="draft-heading"><h2 id="draft-heading">내 보고서</h2><p>자료에서 찾은 근거와 나의 판단을 정리하세요.</p><label htmlFor="draft">보고서 내용</label><textarea id="draft" value={draft.text} readOnly={!writable} onChange={event=>draft.change(event.target.value)} placeholder="문제의 원인과 근거를 작성해 보세요."/><div className={styles.saveRow}><span>{count.toLocaleString()} / 20,000자</span><span role={['error','conflict','locked','reload-error'].includes(draft.status)?'alert':'status'}>{labels[draft.status]}</span>{writable&&<button disabled={!draft.dirty||draft.status==='saving'||['conflict','reload-error'].includes(draft.status)||count>20000} onClick={()=>draft.status==='error'?draft.retry():void draft.save()}>{draft.status==='error'?'저장 재시도':'지금 저장'}</button>}</div>{count>20000&&<p role="alert">보고서는 20,000자 이내로 작성해 주세요.</p>}{['conflict','reload-error'].includes(draft.status)&&<button onClick={()=>void draft.reload()}>서버 저장본 불러오기</button>}<p className={styles.note}>입력 후 자동 저장됩니다. 최초 제출 후에는 제출본을 변경할 수 없습니다.</p></section>;
 const assistant=<ChatPanel sessionId={workspace.session.id} userId={userId} allowed={writable&&workspace.session.allowedActions.includes('SEND_MESSAGE')} draftReady={!draft.dirty&&draft.status==='saved'} onBusy={setChatBusy}/>;
 return flow?<WorkPanels reference={reference} artifact={artifact} assistant={assistant}/>:<><nav><Link href={`/tasks/${workspace.task.id}`}>과제 소개로 돌아가기</Link></nav><div className={styles.heading}><p>자료를 읽고, 내 근거로 정리하기</p><h1>{workspace.task.title}</h1><span>보고서 작성</span></div><div className={styles.columns}>{reference}{artifact}</div>{assistant}{<SubmissionPanel sessionId={workspace.session.id} userId={userId} draft={draft.snapshot} ready={!chatBusy&&!draft.dirty&&draft.status==='saved'} allowed={workspace.session.allowedActions.includes('SNAPSHOT_INITIAL')} onBusy={setSubmitting} onSealed={markSealed} onReload={draft.reload}/>}</>;
}
