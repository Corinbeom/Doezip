...
- P01 /learn은 기존 편집기를 재사용하되 새 흐름의 제출 API로 연결한다. 화면에 표시된 artifact version/hash를 사용하고 다른 창의 최신 본문을 몰래 제출하지 않는다. 직접 설명 단계에서는 AI 패널을 숨긴다. 검증 설명은 명시적 저장이며 미저장 내용을 새로고침 복원한다고 표시하지 않는다.
- 과제 시작 요청은 catalog 응답의 `catalogId`와 `version`을 그대로 사용한다. 화면의 유형 라벨을 서버 과제 선택 값으로 바꾸거나 최신 버전을 클라이언트에서 추정하지 않는다.

- P02 단계·패널 이동은 숨김으로 편집기 생명주기를 유지한다. 단계 이동과 실제 제출을 구분한다. 훈련 안내는 고정 가이드이며 행동 분석 결과로 표시하지 않는다. reduced-motion을 존중하며 기존 단독 작업 화면을 회귀 검사한다.

- P03 훈련 힌트는 한 번에 한 주제를 펼치고 열람 수만 안내한다. 근거 연결은 실제 자료 문장을 먼저 읽고 연속된 줄을 선택하게 한다. 피드백은 요청 구체화→근거 확인→결과물 개선→판단 설명 순서의 집중 보기와 전체 기록을 함께 제공하며 평가 원문을 변형하지 않는다.
