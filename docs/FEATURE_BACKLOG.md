...
2. **F01 사용자 식별·소유권:** 기존 인증 계약을 따라 실제 사용자를 식별하고 자기 리소스만 접근하도록 검증한다. 외부 계정 준비가 필요하면 해당 시점에 확인하며 임의 인증 우회를 제품에 추가하지 않는다.
...
현재 F02a는 로컬 구현·검증을 완료했고 develop 통합은 아직이며 나머지 기능은 **미구현**이다. [F02a 기록](F02A_TASK_BROWSE.md)을 참고한다. 1인 개발의 검토·병합 규칙은 [기여 가이드](../CONTRIBUTING.md)를 따른다.
