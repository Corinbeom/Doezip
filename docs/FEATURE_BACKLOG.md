...
1. **F02a 과제 목록·상세 조회 — 다음 구현:** 기존 공개 tasks 계약에 맞춰 필요한 콘텐츠 테이블만 Flyway로 만들고, 검수한 가상 과제 1개를 저장한다. 기술 확인용 최소 화면 → 실제 API → PostgreSQL 조회를 연결한다. 목록·상세·없는 과제·비공개 자료 미노출을 테스트한다. 전체 ERD 및 seed 완료로 표시하지 않는다.
...
현재 이 기능들은 **미구현**이다. 1인 개발의 검토·병합 규칙은 [기여 가이드](../CONTRIBUTING.md)를 따른다.
