...
3. **F02b 작성·저장·복원:** 과제 시작 → 자료 열람 → 보고서 작성 → DB 저장 → 새로고침 복원. 저장 실패·충돌·타인 접근을 검증한다.
...
현재 F02a는 로컬 구현·검증을 완료했고 develop 통합은 아직이다. F01은 로그인·사용자 매핑 코드와 소유권 검사 공통 기반을 구현했으며 최초 Google 로그인은 사용자 완료 확인과 DB 조회로 확인했다. 새로고침 상태 유지·로그아웃 후 재로그인은 사용자 확인과 DB 중복 검사로 확인했다. 실제 토큰 갱신·두 계정 검증은 남아 있다. 도메인별 소유권 적용과 F02b 이후 기능은 미구현이다. [F01 기록](F01_AUTH_VALIDATION.md)을 참고한다. [F02a 기록](F02A_TASK_BROWSE.md)을 참고한다. 1인 개발의 검토·병합 규칙은 [기여 가이드](../CONTRIBUTING.md)를 따른다.
