...
## 현재 상태 — 1인 개발 (2026-09-17)

- F00~F05c와 I01은 `develop`에 통합됐다.
- I02~P05의 통합 학습 흐름, UI QA 개선, 우측 AI 대화 사이드바는 PR #4로 `develop`에 반영됐다.
- P06은 기존 완료 기록을 보존하는 콘텐츠 v0.2를 추가한다. 보고서 자료 세 개와 구현 경계 조건을 제공하며 상세 범위는 [P06 기록](P06_CONTENT_V2.md)을 따른다.
- 공개 데모는 Vercel·Render에 있으나 최신 `main` 반영과 운영 smoke 결과를 배포 기록에서 별도로 확인한다.
- 후속 우선순위는 실제 사용자 관찰, 과제 다양화, 평가 의미 품질 검토, 무료 Render의 cold start·DB 수명 대응이다. 전체 ERD 22개와 범용 코드 실행 환경은 완료로 표시하지 않는다.

## 과거 실행 순서 — 1인 개발 전환 시점 (2026-09-10)
...
사용자 지시(2026-09-15)로 외부 Git 제출 대신 단일 JavaScript 과제를 서비스에서 편집·AI 수정·실행·제출하는 흐름을 우선한다.
구현/검증 범위는 [F08a](F08A_CODING_WORKSPACE.md). 여러 파일·언어, 자율 실행 에이전트, 독립 서버 채점과 AI 역량 평가는 후속이다.

## I02 보고서·구현 통합
두 기능을 같은 origin/API/로그인/DB에서 실행한다. [통합 범위](I02_UNIFIED_WORKSPACE.md). 평가 입력 확장이나 다른 미커밋 기능 통합은 별도다.

## P01 흐름 개정
사용자가 합의한 두 유형·두 모드의 수행 흐름은 [P01](P01_LEARNING_FLOW.md)에서 관리한다. F05d의 이전 검산 기반 품질 조정을 새 흐름 확정 전에 계속 확장하지 않는다. 기존 검산·평가 경로는 이전 세션 호환용이며 새 과제의 필수 단계가 아니다.

## P02 단계형 작업 공간
과제 이해/작업/검증 단계 이동, 자료·AI·편집 패널, 훈련 단계별 안내, 제출 확인과 입력 보존. [동작·검증 범위](P02_GUIDED_WORKSPACE.md). 개인화 AI 코칭·시간제한·감독 기능은 제외.

## P08 통합 과제 카탈로그

보고서와 구현 과제를 `/tasks`에서 함께 탐색하고 유형·난도·예상 시간·태그를 비교한다. 독립 `/coding` 목록은 제거하되 기존 상세 기록 URL은 보존한다. catalogId와 콘텐츠 version은 과제 표시·후속 버전 관리의 기준이며 문제 풀 확대와 과제별 버전 선택은 후속 문제 품질 작업으로 남긴다. [구현 범위](P08_UNIFIED_TASK_CATALOG.md)를 따른다.
