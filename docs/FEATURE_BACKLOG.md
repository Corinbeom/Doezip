...
현재 F02a는 로컬 구현·검증을 완료했고 develop 통합은 아직이다. F01은 로그인·사용자 매핑 코드와 소유권 검사 공통 기반을 구현했으며 최초 Google 로그인은 사용자 완료 확인과 DB 조회로 확인했다. 새로고침 상태 유지·로그아웃 후 재로그인은 사용자 확인과 DB 중복 검사로 확인했다. 실제 토큰 갱신·두 계정 검증은 남아 있다. F02b에서 세션·자료·draft의 소유권을 적용하며 [F02b 기록](F02B_REPORT_DRAFT.md)에 검증 결과를 관리한다. F02c는 최초 제출과 제출본 조회의 로컬 구현·검증을 완료했으며 develop 미통합이다. F04a 검산 시작·열람은 로컬 구현·검증 완료, develop 미통합이다. F04b 검토 저장·제출은 로컬 구현·검증을 완료했으며 [F04b 기록](F04B_CHALLENGE_REVIEW.md)에서 검증 상태를 확인한다. 평가 요청·상태 처리는 F05a, 결과 검증·저장·조회 기반은 F05b이며 실제 평가기는 미연결이다. [F02c 기록](F02C_INITIAL_SUBMISSION.md)을 참고한다. [F01 기록](F01_AUTH_VALIDATION.md)을 참고한다. [F02a 기록](F02A_TASK_BROWSE.md)을 참고한다. 1인 개발의 검토·병합 규칙은 [기여 가이드](../CONTRIBUTING.md)를 따른다.
