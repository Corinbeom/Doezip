...
| 사용자 기록 보존·삭제 | 탈퇴 전까지 보유, 탈퇴 시 인증 사용자와 소유 학습 데이터 삭제, 재생성 차단값 최대 24시간 | 실제 Supabase 탈퇴 검증 전 |
...
- ADR-P01 (2026-09-15): 사용자 합의에 따라 보고서·구현 두 유형, 훈련·모의 전형 두 모드를 공통 흐름으로 제공한다. 내 결과물 검증 → 제출 → 직접 설명 → 근거 중심 피드백 → 별도 재연습을 사용하고 총점과 별도 검산 필수 경로를 제거한다. 기존 세션에는 적용하지 않는다. 세부 동작과 한계는 P01_LEARNING_FLOW.md를 따른다.

## P02: 작업 공간과 안내 시점
사용자 승인에 따라 작업 단계·좌우 패널·모바일 탭을 사용하고 입력을 보존한다. 훈련은 단계별 안내, 모의 전형은 동일 목표·제출 조건과 독립 수행을 제공한다. 평가·DB 규칙은 P01 유지. [상세 기준](P02_GUIDED_WORKSPACE.md).

## P03: 원문 중심 검증과 단계별 피드백 읽기
훈련 힌트는 세 주제 중 하나만 펼쳐 작업 문맥을 유지하고, 모의 전형에서는 제공하지 않는다. 근거 연결은 줄 번호 직접 입력 대신 실제 자료의 연속된 문장을 선택한다. 평가 결과는 요청 구체화→근거 확인→결과물 개선→판단 설명 순서의 집중 카드와 전체 기록으로 함께 제공한다. 두 화면은 동일한 저장 결과를 투영하며 별도 AI 호출이나 새 평가를 만들지 않는다. 캐릭터·브랜드 심볼은 별도 디자인 작업으로 남긴다. [상세 기준](P03_FEEDBACK_EXPERIENCE.md).

## ADR-P13: 운영 정책 공개와 확인 기록 (2026-09-18)

- `/terms`, `/privacy`, `/ai-policy`를 로그인 없이 공개하고 공통 푸터에서 연결한다.
- Google 인증 뒤 현재 세 정책 버전을 모두 확인해야 학습 화면으로 이동한다. 기존 사용자에게 과거 동의를 임의로 채우지 않는다.
- 서버는 정책 버전 세 개와 확인 시각을 함께 저장하며 일부 필드만 존재하는 상태를 DB 제약으로 막는다. 정책이 바뀌면 서버·웹 버전과 문서를 함께 갱신한다.
- Gemini 데이터 안내는 배포 환경의 실제 유·무료 처리 조건을 표시한다. 개인정보 문의 주소와 Supabase 처리 지역은 운영자가 공개 전에 실제 설정으로 확정한다.
- 이 흐름은 웹의 이용 안내 및 확인 기록이다. 개별 제품 API 전체에 정책 확인을 인가 조건으로 강제하는 기능은 이번 범위에 포함하지 않는다. [상세 기준](P13_LEGAL_PRIVACY.md).
- 내 학습에서 명시적 확인 후 `DELETE /api/v1/me`를 호출한다. 외부 인증 삭제가 실패하면 로컬 기록을 보존하고, 성공하면 사용자 소유 기록을 연쇄 삭제한다. 공유 과제 콘텐츠는 삭제하지 않는다.
- Supabase 삭제 후 이미 발급된 JWT는 만료 전까지 유효할 수 있으므로 단방향 변환한 인증 식별자를 최대 24시간 보관해 계정 자동 재생성을 차단한다.
